...
# Ben's injection times for O1
#for INJECTION_TIME in 1126253357; do
#for INJECTION_TIME in 1126253357 1126254257 1126255157 1126256417 1126257557 1126260497 1126266197 1126261397 1126264997 
#for INJECTION_TIME in 1126297757 1126294817 1126296497 1126311917 1126313117 1126315517 1126316717; do #1126253357

#for INJECTION_TIME in 1126255157; do
#for INJECTION_TIME in 1126253357 1126254257 1126255157 1126256417; do
#for INJECTION_TIME in 1126257557 1126260497 1126266197 1126261397; do 
#for INJECTION_TIME in 1126264997 1126297757 1126294817 1126296497; do
#for INJECTION_TIME in 1126311917 1126313117 1126315517 1126316717; do
#for INJECTION_TIME in 1126260497 1126266197 1126261397 1126264997 1126297757 1126294817 1126296497 1126311917 1126313117 1126315517 1126316717; do
# L1 injection times in O2
...
#for INJECTION_TIME in 1168927750 1168928750 1168932750 1168933750; do # 1168928750 1168932750 1168933750; do
for INJECTION_TIME in 1168934750 1168935750 1168936750 1168937750; do

# Couple extra O2 times for L1
#for INJECTION_TIME in 1168938750 1168955750; do

# times to analyze
START_TIME=$((${INJECTION_TIME} - 750))
END_TIME=$((${START_TIME} + 2048))

# sleep 15 minutes to allow previous workflow's frame writing to finish
if [[ ${LOOPCOUNT} != 0 ]]; then
...
EVENT=170104 # bens 150914 #170104
MODEL=O2
IFO=H1
...
CONFIG_PATH=${PWD}/config_o2_detune_${IFO}.ini
...
CONFIG_PATH=${PWD}/config_detune_${IFO}.ini
FRAME_TYPE=${IFO}:${IFO}_HOFT_C01
CHANNEL_NAME=${IFO}:DCS-CALIB_STRAIN_C01
fi

WORKFLOW_NAME=cal_${INJECTION_TIME}_${EVENT}_${IFO}_${MODEL}model_detune

if [[ ${EVENT} == 170104 ]]; then
echo Making 170104 injection at ${INJECTION_TIME}
...
echo Making 150914 injection at ${INJECTION_TIME}
...
echo "Making Ben's injection at ${INJECTION_TIME}"
INC=0.0
POL=0.0
RA=1.0
DEC=1.0
...
pycbc_generate_hwinj --approximant SEOBNRv2 --order pseudoFourPN --mass1 ${M1} \
    ...
    --psd-segment-length 16 --psd-segment-stride 8 \
...
#HWINJ_PATH=${PWD}/`ls ${IFO}-HWINJ_CBC*.xml.gz`
...
#/home/cbiwer/opt/pycbc-dev/bin/pycbc_submit_dax --no-create-proxy --dax ${WORKFLOW_NAME}/${WORKFLOW_NAME}.dax --accounting-group sugwg.astro
